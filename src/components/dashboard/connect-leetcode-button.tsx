...
type ConnectLeetCodeButtonProps = {
  username?: string | null;
};

export function ConnectLeetCodeButton({ username }: ConnectLeetCodeButtonProps) {
  ...
  useEffect(() => {
    ...
    return undefined;
  }, [isDialogOpen, username, isMounted]);
  ...
  const handleSubmit = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!USERNAME_PATTERN.test(normalized)) {
      setInputError("Use 3-20 characters with letters, numbers, underscores, or dashes.");
      return;
    }
    ...
    startTransition(async () => {
      ...
      try {
        const response = await fetch("/api/integrations/leetcode", {
          method: "POST",
          headers: {
        ...
        if (!response.ok) {
          ...
          throw new Error(payload?.error ?? "Failed to connect LeetCode");
        }

        setIsDialogOpen(false);
        router.refresh();
      } catch (err) {
        setRequestError(err instanceof Error ? err.message : "Unable to connect LeetCode");
      }
    });
  };
      ...
      {isMounted && isDialogOpen
        ? createPortal(
            <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/50 p-4">
              ...
              >
                <h3 className="text-lg font-semibold text-[#0f172a]">Connect LeetCode</h3>
                <p className="mt-1 text-sm text-neutral-500">
                  Add your LeetCode username to import solved problems, contest stats, and badges.
                </p>

                <form className="mt-6 space-y-4" onSubmit={handleSubmit}>
                  <div className="space-y-2">
                    <label className="block text-sm font-medium text-neutral-700" htmlFor="leetcode-username">
                      LeetCode username
                    </label>
                    <input
                    ...
                    <p className="text-xs text-neutral-500">
                      3-20 characters. Letters, numbers, underscores, and dashes are allowed.
                    </p>
