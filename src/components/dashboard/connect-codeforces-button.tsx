...
type ConnectCodeforcesButtonProps = {
  handle?: string | null;
};

export function ConnectCodeforcesButton({ handle }: ConnectCodeforcesButtonProps) {
  const router = useRouter();
  const [isMounted, setIsMounted] = useState(false);
  const [isDialogOpen, setIsDialogOpen] = useState(false);
  const [candidateHandle, setCandidateHandle] = useState(handle ?? "");
  ...
  useEffect(() => {
    ...
    return undefined;
  }, [handle, isDialogOpen, isMounted]);

  const handleSubmit = (event: React.FormEvent<HTMLFormElement>) => {
    event.preventDefault();

    const normalized = candidateHandle.trim();

    if (!normalized) {
      setInputError("Handle cannot be empty.");
      return;
    }

    if (!HANDLE_PATTERN.test(normalized)) {
      setInputError("3-24 characters. Letters, numbers, underscores, dashes, and dots are allowed.");
      return;
    }
    ...
    startTransition(async () => {
      ...
      try {
        ...
        if (!response.ok) {
          ...
          throw new Error(payload?.error ?? "Failed to connect Codeforces");
        }

        setIsDialogOpen(false);
        router.refresh();
      } catch (err) {
        setRequestError(err instanceof Error ? err.message : "Unable to connect Codeforces");
      }
    });
  };
    ...
    <>
      <div className="flex flex-col items-end gap-2">
        ...
        >
          ...
          {isPending ? "Saving…" : handle ? "Update handle" : "Connect"}
        </button>
      ...
      {isMounted && isDialogOpen
        ? createPortal(
            <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/50 p-4">
              ...
              >
                <h3 className="text-lg font-semibold text-[#0f172a]">Connect Codeforces</h3>
                <p className="mt-1 text-sm text-neutral-500">
                  Add your Codeforces handle to display rating, rank, and latest contest results.
                </p>

                <form className="mt-6 space-y-4" onSubmit={handleSubmit}>
                  <div className="space-y-2">
                    <label className="block text-sm font-medium text-neutral-700" htmlFor="codeforces-handle">
                      Codeforces handle
                    </label>
                    <input
                    ...
                    <p className="text-xs text-neutral-500">
                      3-24 characters. Letters, numbers, underscores, dashes, and dots are allowed.
                    </p>
