...
type ConnectGeeksforgeeksButtonProps = {
  username?: string | null;
};

export function ConnectGeeksforgeeksButton({ username }: ConnectGeeksforgeeksButtonProps) {
  ...
  const handleSubmit = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!USERNAME_PATTERN.test(normalized)) {
      setInputError(
        "3-30 characters. Letters, numbers, underscores, dots, and dashes are allowed."
      );
      return;
    }
    ...
    startTransition(async () => {
      ...
      try {
        ...
        if (!response.ok) {
          ...
          throw new Error(payload?.error ?? "Failed to connect GeeksforGeeks");
        }
        ...
        setRequestError(
          err instanceof Error ? err.message : "Unable to connect GeeksforGeeks"
        );
      }
    });
  };
      ...
      {isMounted && isDialogOpen
        ? createPortal(
            <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/50 p-4">
              ...
              >
                <h3 className="text-lg font-semibold text-[#0f172a]">Connect GeeksforGeeks</h3>
                <p className="mt-1 text-sm text-neutral-500">
                  Add your GeeksforGeeks username to highlight coding score, solved problems, and ranks.
                </p>

                <form className="mt-6 space-y-4" onSubmit={handleSubmit}>
                  <div className="space-y-2">
                    <label className="block text-sm font-medium text-neutral-700" htmlFor="gfg-username">
