...
export function DisconnectCodechefButton() {
    ...
    startTransition(async () => {
      try {
        ...
        if (!response.ok) {
          ...
          throw new Error(payload?.error ?? "Failed to disconnect CodeChef");
        }

        toast.success("CodeChef disconnected");
        router.refresh();
      } catch (err) {
        toast.error(err instanceof Error ? err.message : "Unable to disconnect CodeChef");
      }
    });
  };
