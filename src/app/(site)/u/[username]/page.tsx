...
export default async function ProfilePage({ params }: ProfilePageProps) {
  ...
  const contributionsPromise = getContributionSeriesForUser(user).catch(
    (error) => {
  ...
  const dateRange = {
    start:
      contributionResult?.series.startDate ??
      new Date(new Date().getFullYear(), 0, 1).toISOString().split("T")[0],
    end:
      contributionResult?.series.endDate ??
  ...
  const activityStats = contributionResult?.stats;
        ...
        <header className="card space-y-6 p-10 md:w-[35vw]">
          <div className="flex flex-col gap-4 ">
            <div className="space-y-2">
              <p className="text-5xl bg-teal-600 flex items-center justify-center text-white w-20 h-20 rounded-xl border-2 border-teal-900">{displayName[0]}</p>
          ...
          <div className="flex flex-col gap-4 ">
            <p className="text-sm text-neutral-500 md:max-w-2xl">
              AyyCode pulls together contributions across coding platforms. Link
              services from your dashboard to turn this space into a living
              portfolio of your progress, contests, and repositories.
            </p>
          </div>
        </header>
      ...
      <section className="space-y-6">
        <div className="space-y-2">
          ...
          <p className="text-sm text-neutral-500">
            Stats update automatically once a service is linked. Until then, use
            the connect prompts to wire everything up.
          </p>
        </div>
